...
class IncidentReporter(BaseReporter):
    ...
    def _executive_summary_section(self) -> List[str]:
        s = self.session
        lines = ["## Executive summary", ""]
        verified = len(self.ctx.verification_commands) > 0
        failed = len(self.ctx.failed_commands)
        parts = [f"Incident note for \"{s.title}\"."]
        parts.append(
            f"{s.summary.commands_count} command(s) and "
            f"{s.summary.notes_count} note(s) were recorded."
        )
        if failed:
            parts.append(f"{failed} command(s) failed during the session.")
        parts.append(
            "At least one verification command passed."
            if verified
            else "No verification command passed."
        )
    ...
    def _timeline_section(self) -> List[str]:
        lines = ["## Chronological event timeline", ""]
        if not self.ctx.timeline:
            lines.append("_No events were recorded._")
            return lines
        for entry in self.ctx.timeline:
            lines.append(f"- `{_clock(entry.timestamp)}` **{entry.kind}**: {entry.text}")
        return lines

    def _actions_section(self) -> List[str]:
        lines = ["## Actions taken", ""]
        if not self.ctx.report_commands:
            lines.append("_No commands were recorded as actions._")
            return lines
    ...
    def _followup_section(self) -> List[str]:
        ...
        if not items:
            items.append("No automatic follow-up items were derived.")
        for item in items:
            lines.append(f"- {item}")
        return lines
